#Adblock library manager v0.9
import os
import csv
full_library_name = os.path.join("/Volumes/Liam's Works/Programming/Python/Adblock Library Project/","community-enhanced-adblock-library.txt")
...
contributions_library_file = open(full_library_name, "r")
    ...
    if edit_or_print == "PRINT":
        ...
        while selected2 == False:
            ...
            if user_input.upper() == "C" or user_input.upper() == "CONTRIBUTIONS" or user_input.upper() == "CONTRIBUTIONS LIBRARY":
                for i in range(len(contribution_rows)-2):
                    print(contribution_rows[i+2][:-1])
                print(contribution_rows[-1])
            ...
            if user_input.upper() == "C" or user_input.upper() == "CONTRIBUTIONS" or user_input.upper() == "CONTRIBUTIONS LIBRARY":
                ...
                while selected6 == False:
                    ...
                    if user_input.upper() == "M" or user_input.upper() == "MANUALLY" or user_input.upper() == "MANUALLY INPUT":
                        selected6 = True
                        print("Paste elements to add them to the submissions library.")
                        print("Then click 'enter' to add them to official library, type 'clear' to clear the library, type 'end' to end the program.")
                        library_file = open(full_contributions_library_name, "a")
                        user_input = input()
                        while user_input.upper() != "END":
                            if user_input.upper() == "CLEAR":
                                library_file.truncate(0)
                                #delete items in official library
                                library_file.writelines(contribution_rows[:1])
                            elif user_input != "":
                                library_file.write("\n"+user_input)
                                contribution_rows.append(user_input)
                            user_input = input()
                        library_file.close()
                        ...
                        for row in csv_reader:
                            contribution_rows.append(row)
                        csv_file.close()
                        for i in range(len(contribution_rows)):
                            if contribution_rows.count(contribution_rows[i]) >= 3:
                                deleting_item = str(contribution_rows[i])
                                library_file = open(full_library_name, "a")
                                official_library.append(contribution_rows[i])
                                library_file.close()
                                for i in range(len(contribution_rows)):
                                    if str(contribution_rows[i]) == deleting_item:
                                        del contribution_rows[i]
                                contributions_library_file = open(full_library_name, "a")
                                contributions_library_file.truncate(0)
                                contributions_library_file.writelines(contribution_rows)
